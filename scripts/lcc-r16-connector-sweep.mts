...
function scanAnchored(
  ...
  mode: 'type' | 'serial',
  capWordOnly: boolean,
  split: RegExp = CONNECTOR_SPLIT,
) {
  const found = new Set<string>();
  for (const raw of segmentsOf(window, split)) {
...
const connectorOf = (w: string, list = CONNECTORS) =>
  list.find((c) => c.re.test(w))?.name ?? null;

/**
 * R16 — `AND`, THE CONJUNCTION THE CENSUS NEVER ASKED ABOUT.
 *
 * `docs/ai/lcc-r16/residue.json` hand-read the 22 residue bearers that DO name
 * their sibling, and the single commonest shape is a second cause title opened
 * by `AND`:
 *
 *   AND Case :- APPLICATION U/S 482 No. - 2283 of 2023
 *   AND CIVIL APPLICATION NO.738 OF 2021
 *   AND FIRST APPEAL NO.884 OF 2023
 *
 * `AND` is not in the shipped connector list. It was never censused — R15-F1
 * removed `ANALOGOUS` and `TAGGED WITH` for firing zero times and added
 * nothing, so this is a gap rather than a rejected candidate.
 *
 * It is also the most dangerous word that could be added, because `and` joins
 * ordinary prose. That is exactly why it is swept in three named shapes rather
 * than simply added: anywhere, at the start of a line only, and at the start of
 * a line in capitals only. The question is which SHAPE the courts print, and a
 * sweep that only widened a bound would not answer it.
 */
const AND_ANYWHERE = [...CONNECTORS, { re: /\band\b/i, name: 'AND' }];
const AND_LINE = [...CONNECTORS, { re: /(^|\n)\s*and\b/i, name: 'AND' }];
const AND_LINE_UPPER = [...CONNECTORS, { re: /(^|\n)\s*AND\b/, name: 'AND' }];
const SPLIT_AND_ANYWHERE = new RegExp(CONNECTOR_SPLIT.source + String.raw`|\band\b`, 'gi');
const SPLIT_AND_LINE = new RegExp(CONNECTOR_SPLIT.source + String.raw`|(?<=^|\n)\s*and\b`, 'gi');
const SPLIT_AND_LINE_UPPER = new RegExp(CONNECTOR_SPLIT.source + String.raw`|(?<=^|\n)\s*AND\b`, 'g');
...
const VARIANTS: Record<string, (head: string) => boolean> = {
  ...
  // ── R16 arms: the shipped grammar (V3), plus `AND` in three shapes ──
  V6_SHIPPED_PLUS_AND_ANYWHERE: (h) => {
    const w = h.slice(0, W);
    return (
      connectorOf(w, AND_ANYWHERE) !== null &&
      scanAnchored(w, 'serial', false, SPLIT_AND_ANYWHERE) > 1
    );
  },
  V7_SHIPPED_PLUS_AND_LINE_START: (h) => {
    const w = h.slice(0, W);
    return (
      connectorOf(w, AND_LINE) !== null && scanAnchored(w, 'serial', false, SPLIT_AND_LINE) > 1
    );
  },
  V8_SHIPPED_PLUS_AND_LINE_START_UPPER: (h) => {
    const w = h.slice(0, W);
    return (
      connectorOf(w, AND_LINE_UPPER) !== null &&
      scanAnchored(w, 'serial', false, SPLIT_AND_LINE_UPPER) > 1
    );
  },
};
...
writeFileSync('docs/ai/lcc-r16/connector-sweep.json', JSON.stringify(out, null, 2) + '\n');
console.log('\nwrote docs/ai/lcc-r16/connector-sweep.json');
await sql.end();
