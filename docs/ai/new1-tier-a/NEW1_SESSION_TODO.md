...
| L5 | Act on NEW2 0072 — `TEXT_UNSAFE` must not fire for exact-route classes | **DONE** | commit `314b22c`. NEW2 was right; the first cut would have sent them to OCR documents whose identity fields were never damaged |
| L6 | Sweep the 2,530 staged vectors on newly-convicted documents | **QUEUED, gate-waiting** | needs no new tool — `stage-quarantine-refused.mjs --text-unsafe` is re-runnable and reads the deployed view. Waiting on DB_SCAN, which has read DEFER all night (longest statement 205–5,636 s). `gated-run.sh` shouts if it gives up |
| L7 | P1 — run the frozen benchmark through the app | **CITATION CLASS DONE** | **s@1 97.38% · s@5 99.13% · MRR 0.9807 · p50 5 ms · p95 39 ms · 0 timeouts · 0 non-200**, 229 queries in 21 s. The public-V1 gate "citation search works" is MET on the production path. The 2 failures are not ranking failures — see L10 |
| L7b | The other three classes | DEFERRED, with a reason | `case_title` and the two semantic classes still pay the 53 s sparse arm, so 800 queries is ~7 h and would starve the walk. The semantic classes are also ceiling-bound at 0.5–1.5% (L4), so running them through production costs hours to confirm ~0. The counterfactual was measured instead — L8 |
| L8 | P3/P7 — would the document vectors answer what production cannot | **DONE** | dense arm alone against `new1_probe_half_250k`, 571 queries. `fact_passage` s@1 9.58% · s@5 15.21% · s@20 20.56%. `nl_doctrine` s@1 9.23% · s@5 13.33% · s@20 16.41%. **UPPER BOUND** three ways. Against a production ceiling of 0.5–1.5%, wiring them in is necessary and NOT sufficient |
| L9 | P8 — hand NEW3/RCC one stable search contract | **DONE** | `docs/ai/new1-tier-a/SEARCH_CONTRACT_FOR_PRODUCT.md`, bus 1010/1011. A behavioural annex; `API_CONTRACTS.md` §Search is LCC's and untouched |
| L10 | Citation uniqueness — found by the 2 citation failures | **MEASURED, handed over** | **>= 100,000** neutral citations name more than one judgment (a floor — the count was capped). 97.0% of a 3,000-group sample hold >1 case number; worst group **303**. Allahabad 16,229 groups of the first 20,000. **Whether this is the court's batch numbering or our extractor is UNKNOWN and I did not assert either** — needs source PDFs read against the registry. NEW2 bus 1017, LCC bus 1016 |
| L12 | **case-name search fails its gate at 40.61% s@1 — and the cause is the word "AND"** | **CAUSE PROVEN, fix landed, re-measure pending** | `looksStructured` treats a bare uppercase `AND` as a boolean operator, and Indian titles say `AND OTHERS` / `AND ANOTHER` / `AND 4 OTHERS`. **100 of 229 (43.7%)** were parsed as boolean expressions, matched nothing, and the route returned ZERO in ~2 ms with no search performed. Cross-tabulated rather than inferred from matching magnitudes: **misrouted n=100 found 4 (4.0%) median 2 ms · not misrouted n=129 found 97 (75.2%) median 2,559 ms** |
| L12b | **PREDICTION, recorded BEFORE the re-run** | pending | if the mechanism is the whole story, the fix moves `case_title` s@20 from **44.1% to ~75%** (the 100 misrouted rows should behave like the 129 that were never misrouted). A materially different number means the mechanism is only part of it, and that is the point of writing the number down first |
| L11 | The disambiguation list can omit the answer | **MEASURED, handed to LCC** | `runStructured` is `ORDER BY judgment_date DESC LIMIT 5` and the route passes `RESULT_LIMIT = 5`. `2026:PHHC:027747-DB` resolves to 15; the right case is not in the 5 shown. 2 of 229 (0.9%). Not touched — `RESULT_LIMIT` is LCC's and the fix is a product call |
