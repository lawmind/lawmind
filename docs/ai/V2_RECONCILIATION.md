...
| V2 item | Gap |
| --- | --- |
| **§3 Rule 1 / Rule 6, ambiguity** | **Built, 11 Aug 2026.** `StructuredOutcome` gained an `ambiguous` kind; `cite:"2020 INSC 189"` (three real judgments, same date, same court) now returns it, verified live against production. `docs/CITATION_HARNESS.md`. |
| §7 Production integrity, `/version` | **Built, 11 Aug 2026** — `GET /version` returns `gitSha`/`deployedAt`/`environment`/`imageDigest`. Fixed the deeper bug alongside it: `GIT_SHA` was a hand-set variable from 8 Aug, never updated by CLI (`railway up`) deploys; `scripts/deploy-api.mjs` now sets it atomically as part of the deploy action. Verified: `/version` in production returns the exact deployed commit. |
...
| §7 Continuous production probes | 4 of 9 listed probes exist. `deployed-safety.ts` (`/search`): impossible citation, real citation, missing `parsed`, ambiguous citation. `deployed-judgment-safety.ts` (`GET /judgments/:id`, added 11 Aug): citationless judgment round-trips null/empty, overruled judgment reports its true status — both PASS against production today. Still missing: evidence-retrieval probe, rendering probe (client-side, RCC's surface). |
| §14 Paragraph-level structure | **Resolved differently than assumed, 11 Aug 2026.** The original entry guessed character offsets were the blocker; they were not needed. `citesJudgmentId?: string` ships on `paragraphs[]` by resolving each paragraph's first citation-shaped span through the same match `cite:` search uses, never guessing on zero or ambiguous matches. RCC notified (bus 0033). |
