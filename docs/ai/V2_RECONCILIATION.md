...
| §7 Fail-closed startup preflight | **Not built.** Nothing refuses to boot if citation resolution is broken. The actual 8–11 Aug incident was fail-*crashed* (container never started, old version kept serving) rather than fail-*open* in V2's sense — but that was accidental (an unrelated missing-module crash), not a designed safety check. A real preflight is still a gap: a future change that breaks `answerStructured` *without throwing* would currently boot and serve degraded. |
