...
| **N2-1** | **Treatment provenance CONSUMER CONTRACT** for LCC-3 | P0 | **DONE · v1.1 CORRECTED** | 8 states × 7 cells + machine-readable twin. **My V1 headline "72 of 76 add-to-matter refusals" was WRONG** — NEW3 bus 1151 caught it, I re-derived it through the shipped `precedentialEffectFromEdges`: **3 refusals, all SYNTHETIC fixtures, 0 real**. I counted the column, not the behaviour. Corrections sent 1158/1159/1160 |
...
| **N2-2e** | **A misspelled month escapes LCC's despatch-stamp gate** — found while reconciling 1116 | P0 | **REPORTED · self-corrected** | I first claimed 11 escape (bus 1136). **Wrong — 1 does.** The gate is a 3-letter prefix + `[A-Z]*`, so `JAN`+`URARY` and `SEP`+`TEMEBER` are already refused; only `ARPIL` (a *transposition* in the first 3 letters) escapes. `2011:ARPIL:05` is indexed today and resolves UNIQUE. Replacement rule scored **34/34, 0 false positives, 0 misses** against the corpus token census. Bus 1140 |
...
| **N2-3** | **`quality_screen_runs` migration + import** | P0 | **DONE** | `0086` (renumbered from 0083 — LCC had claimed it) announced (bus 1148/1149) **then** applied. `judgment_quality_contract` proven untouched: 23 cols / sha `29dfd7de29214b4d` before AND after. 3 runs imported, only 1 `covers_corpus` |
| **N2-3b** | Expose `PROVEN_DAMAGED` / `SCREENED_NO_DAMAGE_FOUND` / `NEVER_SCREENED` | P0 | **DONE** | corpus-wide exact: 469,599 · 1,322,722 · **16,906,647** · **16**. Post-run docs: 16 created after, **16 still NEVER_SCREENED, 0 wrongly certified**. Damage refusal unchanged at 1,792,321 either side of the import |
| **N2-3c** | Never call `SCREENED_NO_DAMAGE_FOUND` "clean" | P0 | **DONE** | `scripts/check-screened-not-clean.mjs` over 1,007 shipping files. **Proven non-vacuous**: a planted violation exits 1, and a planted `SCREENED_CLEAN` exits 1. Needs wiring into `ci-local.mjs` (shared, LCC) |
...
| **N2-14** | **Migration journal gap + ordinal collision** — found while resolving 0083 | P0 | **REPORTED** | 4 files past the journal (`_journal.json` stops at idx 82) so a journal-driven restore skips them; live `drizzle.__drizzle_migrations` last wrote **11 Aug**, 58 rows against 83 entries. My file renumbered 0083 → **0086**. Release-blocking for LCC-7; LCC's to fix |
