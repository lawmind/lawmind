...
3. **IPC↔BNS mapping** (C1) — closes the last S1 criterion.
4. **eCourts bulk ingest** — after the enum decision in §2c, and after a real
   captured response exists to parse (`parseCauseList` is still a deliberate
   stub).
