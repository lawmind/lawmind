...
## 6b · FROM THE SECOND DOCUMENT — what it adds beyond the first

Read 9 August 2026. It overlaps the first heavily; **four things in it are new,
and one of those is the most immediately useful page either document contains.**

### 6b.1 · §34 — the guarantee we should actually make. Take this verbatim.

The document says do not claim *"our AI never hallucinates"*, and offers this
instead:
...
**This is the best sentence in either document, and it should become our public
claim.** Three reasons:

1. **It is measurable.** Every clause maps to something we can compute:
   *"supported by a verified source passage"* → FQ-R1's Unsupported Claim Rate ·
   *"generated from verified source metadata"* → already true, and already
   tested · *"abstains or explicitly identifies the uncertainty"* → our
   `unverified` state and the zero silent-drop threshold.
2. **It is survivable.** *"Never hallucinates"* is disproved by one screenshot.
   This claim is about **architecture**, and the architecture is real.
3. **It matches `CLAUDE.md`'s copy rule already** — *"Safe to file", never "we
   verified this"*. Same instinct, and this is the engineering statement behind
   the product wording.

**Compare Bharat.Law's public claim**: *"0 Hallucinated citations · No
hallucinations, ever"*, with no methodology and no number. **That is the claim
this paragraph tells us not to make**, and it is a competitor's most quotable
liability. `BHARAT_LAW_OFFER.md` §8b.2.

**Queued into `FOUNDER_QUEUE.md` as part of FQ-R1**, because the claim is only
honest once the Unsupported Claim Rate exists to back the first clause.

### 6b.2 · §26 query-type routing — a real gap, and it is now half-built

**Confirmed by reading the code, not assumed:** there was **no query
classification anywhere** in `services/api/src`. Every query ran the same
pipeline — sparse, dense, fuse — including a bare `(2019) 4 SCC 221`.

**That is wrong, and expensively so.** A citation is an exact-match problem with
exactly one right answer, held in an indexed column. Nearest-neighbour search
over 1024-dimensional vectors is slower *and less accurate* at finding it:
embedding models are poor at digits, and `(2019) 4 SCC 221` and
`(2019) 4 SCC 212` are different cases sitting almost on top of each other in
vector space. **An advocate who types a citation expects that case, first,
every time.**

`services/api/src/search/query-shape.ts` now classifies a query as
**citation · section · case_name · concept**, with 18 tests. It is a **pure
function** — no database, no model call, no ranking decision — so routing can be
wired and measured separately.

**It does not redefine the citation patterns.** They already exist in
`@lawmind/ingest/citations`, are already narrow enough that prose cannot match,
and `CLAUDE.md` forbids inventing a citation format. The module was added to
that package's `exports` map rather than copied.

**A defect the tests caught, worth recording:** the Act alternation was written
`BNS|BNSS`, and JS regex alternation takes the **leftmost** match rather than the
longest — so `section 103 BNSS` silently reported `BNS`. **A procedure statute
answered for an offences statute**, which is precisely what `DOMAIN_TRUTH.md`
exists to prevent. Longer alternatives now come first.

**Still to do: wire it.** Routing changes retrieval behaviour, so it goes through
the A/B rig like everything else. The asymmetry makes it safe to be strict — a
missed citation is only a slower correct answer, while a wrongly-claimed one
would pin the wrong judgment at rank 1.

### 6b.3 · §5 proposition-level retrieval — genuinely different from decomposition

> *Do not retrieve merely because a document mentions the same words.* First
> identify the **PROPOSITION** — *"whether anticipatory bail can continue
> without a fixed temporal limitation"* — then retrieve authorities supporting
> **or contradicting** it.

This is not query decomposition, which splits a question into sub-questions.
**It reframes the question into the legal proposition at issue**, which is how
an advocate actually searches, and it makes *contradicting* authority a
first-class retrieval target rather than an accident.

**It needs an LLM to extract the proposition**, so it joins the query-side
levers waiting on `OPENROUTER_API_KEY`. Worth ranking above HyDE when that key
lands: HyDE guesses at a document, this names the actual legal question.

### 6b.4 · §1 paragraph-level evidence IDs — the deepest gap, and the most work

The document makes `SC_2024_000184_P087` **the fundamental unit of citation**,
with the chain PDF → page → paragraph → evidence ID.
...
not a paragraph** — it is a retrieval window that can straddle two — so nothing
today gives a citation a stable, immutable pointer to the paragraph a court
printed.

**Everything else the documents ask for depends on this**: claim→evidence
mapping (§11), the evidence viewer's exact passage (§14), and FQ-R1's
claim-support check all need a paragraph ID to point at.

**This is a corpus-wide re-derivation, not a feature**, and it should not start
before Gate S2. Queued as **FQ-R2** so it is not lost.

### 6b.5 · What the second document does NOT change

Its §19 technology layers repeat the first document's — OpenSearch, Qdrant,
Neo4j, Redis, vLLM. **The answer is unchanged and is in §5 above:** written for
20M documents, we hold 38,341, Postgres already does all four jobs, and the
stack is fixed. Its §17 *"evidence engine deterministic, reasoning engine
probabilistic"* is a good statement of what we already do.
