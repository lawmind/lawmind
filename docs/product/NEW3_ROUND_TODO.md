...
## NEW3-7 — product QA of RCC · `[!]`

**Deferred: RCC has landed nothing this sprint.** No CNR/case-number UX, no
premium preview client, no website, no provenance UI exists to review. The specs
they will be reviewed against are finished and delivered (bus 1147), and §8 of
each spec states the acceptance criteria in advance — deliberately, so the bar is
set before the implementation rather than after it.
