...
async function main() {
  ...
  const dateFails: RejEntry[] = rejected.entries.filter((e: RejEntry) =>
    e.reasons.some((r) => r.startsWith('DATE_PLAUSIBILITY_FAIL')),
  );
  ...
  try {
    ...
    for (const e of dateFails) {
      ...
      if (!gold || !citing) {
        stillRejected.push({
          ...e,
          dateQualityEvidence: { error: 'endpoint missing at recheck time' },
        });
        continue;
      }

      const goldQ = dateQuality({
        judgmentDate: gold.judgment_date,
        sourceUrl: gold.source_url,
        text: gold.full_text,
      });
      const citingQ = dateQuality({
        judgmentDate: citing.judgment_date,
        sourceUrl: citing.source_url,
        text: citing.full_text,
      });
      ...
      const correctedPossible =
        goldWitness !== null && citingWitness !== null ? goldWitness <= citingWitness : null;

      const evidence = {
        goldDateQuality: { state: goldQ.state, method: goldQ.method, witnesses: goldQ.witnesses },
        citingDateQuality: {
          state: citingQ.state,
          method: citingQ.method,
          witnesses: citingQ.witnesses,
        },
        goldWitnessDate: goldWitness,
        citingWitnessDate: citingWitness,
        correctedChronologyPossible: correctedPossible,
        recoveryDenied: bothVerified
          ? 'both endpoints DATE_VERIFIED by their own document text -- not a date defect'
          : null,
      };
      ...
      if (!bothVerified && correctedPossible === true) {
        recovered.push(e);
        console.log(
          `RECOVERED ${e.goldJudgmentId.slice(0, 8)}: gold=${goldQ.state}(${goldWitness}) citing=${citingQ.state}(${citingWitness})`,
        );
      ...
      for (const r of rows) {
        v2.rows.push({
          ...
          provenance: {
            ...
            prohibitedFeatureFamilies: [
              'inbound_citation_count',
              'citation_graph_authority_score',
              'pagerank_style_score',
            ],
            ...(r.queryType !== 'proposition'
              ? {
                  redacted: [],
                  redactedReason:
                    "not applicable: the query IS the target judgment's own citation string or case title by construction",
                }
              : {}),
            recovery: {
              ...
              recoveredBy: 'NEW3',
              reason:
                'bus 0954 (NEW2): judgment_date disagrees with an independent witness (filename/document text) on the suspect endpoint; the witness-corrected chronology is possible',
    ...
    v2.distinctGoldAuthorities = new Set(
      v2.rows.map((r: { goldJudgmentId: string }) => r.goldJudgmentId),
    ).size;
    v2.totalRows = v2.rows.length;
    v2.quarantinedAuthorities = stillRejected.length + (rejected.entries.length - dateFails.length);
    v2.quarantinedRows =
      v2.quarantinedAuthorities > 0
        ? rejected.entries
            .filter((e: RejEntry) => !recovered.some((r) => r.goldJudgmentId === e.goldJudgmentId))
            .reduce((n: number, e: RejEntry) => n + e.rowIds.length, 0)
        : 0;
    v2.recoveryPass = {
      appliedAt: new Date().toISOString(),
      method:
        "NEW2's date-quality.ts dateQuality(), re-verified live per authority; recovered only where an independent witness (not the stored date itself) makes the chronology possible AND at least one endpoint is not DATE_VERIFIED",
      candidatesChecked: dateFails.length,
      recovered: recovered.length,
      stillQuarantined: stillRejected.length,
    };

    const otherRejected = rejected.entries.filter(
      (e: RejEntry) => !e.reasons.some((r) => r.startsWith('DATE_PLAUSIBILITY_FAIL')),
    );
