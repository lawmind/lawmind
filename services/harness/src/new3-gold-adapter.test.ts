...
test('the three query types map to three different provenances', () => {
  const path = fixture([
    { id: 'p1', queryType: 'proposition', query: 'a passage about bail', goldJudgmentId: 'g1', relationship: 'cites', provenance: prov({ redacted: ['citation_text:X'] }) },
    { id: 'c1', queryType: 'exact_citation', query: '2025:PHHC:089161', goldJudgmentId: 'g1', relationship: 'cites', provenance: prov() },
    { id: 't1', queryType: 'case_title', query: 'C Vs D', goldJudgmentId: 'g1', relationship: 'cites', provenance: prov() },
  ]);
...
test('the loaded rows refuse the feature that built them', () => {
  const path = fixture([
    { id: 'p1', queryType: 'proposition', query: 'a passage', goldJudgmentId: 'g1', relationship: 'cites', provenance: prov() },
  ]);
...
test('the chronology drop can be turned off, because the fault may be the DATE not the edge', () => {
  const path = fixture([
    { id: 'p1', queryType: 'proposition', query: 'a passage', goldJudgmentId: 'g1', relationship: 'cites', provenance: prov({ citedDate: '2026-01-12' }) },
  ]);
...
test('mojibake is dropped at five control characters, and four is not mojibake', () => {
  ...
  const path = fixture([
    { id: 'bad', queryType: 'proposition', query: ctl(151), goldJudgmentId: 'g1', relationship: 'cites', provenance: prov() },
    { id: 'ok', queryType: 'proposition', query: ctl(4), goldJudgmentId: 'g2', relationship: 'cites', provenance: prov() },
  ]);
  const { rows, dropped } = loadNew3Gold(path);
  assert.deepEqual(rows.map((r) => r.queryId), ['ok']);
...
test('totals count the file as it was, so a shrinking gold set is visible', () => {
  const path = fixture([
    { id: 'p1', queryType: 'proposition', query: 'a', goldJudgmentId: 'g1', relationship: 'cites', provenance: prov({ citedDate: '2026-01-12' }) },
    { id: 'p2', queryType: 'proposition', query: 'b', goldJudgmentId: 'g2', relationship: 'cites', provenance: prov() },
  ]);
